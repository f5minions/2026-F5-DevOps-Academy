...
UDF > Client-vscode > VSCODE > Terminal에서 사용 가능한 VSCode 터미널을 사용하여 배포합니다.

VSCode에서  app-rgb-bulls-k8s-dep-svc.yaml 파일을 생성하고 아래 내용을 붙여넣습니다. 

![VSCode-newfile](images/vscode-newfile.png)

### app-rgb-bulls-k8s-dep-svc.yaml ###
...
VSCode 터미널에서 아래 명령어를 실행하여 app-rgb-bulls-k8s-dep-svc.yaml 파일을 적용합니다.

```bash
kubectl apply -f app-rgb-bulls-k8s-dep-svc.yaml
```

#### 애플리케이션 배포 확인 및 NodePort 서비스의 IP와 포트 확인 ####
...
추가한 Headermod 확장 기능을 사용하여 User 헤더 값을 bluegroup으로 설정하고, Ctrl + Shift + R을 눌러 캐시를 무시하고 브라우저를 새로고침 합니다.
...
User 헤더 값을 bluegreen으로 수정하고, Ctrl + Shift + R을 눌러 캐시를 무시하고 브라우저를 새로고침 합니다.
...
먼저 기존에 구성한 설정의 bluegreen_bullet upstream 설정을 아래 내용으로 변경합니다.

blue 서버의 weight 값을 99, green 서버의 weight 값을 1로 변경합니다.
...
NGINX Plus API를 사용하여 Blue 서버와 Green 서버의 가중치를 50으로 높이고, 브라우저를 새로고침하여 결과를 확인합니다.
...
트래픽 유입 제어를 위해 `down` 또는 `drain` 속성을 사용합니다. `drain`을 설정하면 새로운 요청은 받지 않되, 기존 세션이 유지되어 해당 서버에 고정(Bound)되어 있는 사용자들의 요청은 완료될 때까지 계속 처리해 줍니다. 
